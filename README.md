...
### 3. Deploy Into Another Repository
...
### 4. Export A Clean Release Package

```powershell
.\scripts\export-release-package.ps1
```

The release package excludes `.git/`, `.agents/runtime/`, local Codex
configuration, local environment configuration, local status records, and other
machine-owned state. The generated manifest records workflow version, source
commit, included files, file hashes, and package hash.

## Enterprise Dispatch

Version `2.1.0` adds the Enterprise Dispatch Layer. It is an optional overlay for
larger tasks where the controller should manage department leaders instead of
directly managing every worker.
...
| `docs/agents/*.yaml` | Canonical route, workflow, policy, deployment, verification, schema, MCP, dispatch, model, and version rules. |
