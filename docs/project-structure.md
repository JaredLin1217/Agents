...
6. `docs/agents/policy.yaml` for isolation or boundary claims
7. `docs/agents/verify.yaml` before claims, commits, deployments, or releases
8. `docs/agents/schemas.yaml` for assignments, reports, status, or templates
9. `docs/agents/mcp.yaml` when optional integrations matter
10. `docs/agents/version.yaml` for compatibility
11. `docs/agents/deploy.yaml` for authorized target deployment
## Role Matrix
...
The `v2.2.0` workflow artifact layer is an optional overlay on top of enterprise
dispatch. Simple answer, scoped edit, plain deploy, and release tasks still use
the minimal route from `docs/agents/ai-runtime.yaml` and should not load
organization or workflow artifact files unless the task is about departments,
employees, delegation, model binding, artifact packets, approval gates,
collection, or artifact-backed simulation.
