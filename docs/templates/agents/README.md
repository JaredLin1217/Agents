# Agents Templates
Target-specific starter bundle for authorized target repos.
## What This Is
Only files that require target adaptation. Canonical rules, schemas, scripts, and runbooks are deployed directly from their repository paths so every deployable contract has one source.
## How To Deploy
...
- `template_provider_mode`: full workflow plus these target-specific starters for redeploy.
