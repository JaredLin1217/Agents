...
Token-min router for isolation, memory, deployment, multi-agent, handoff, skills, maintenance. Start `docs/agents/ai-runtime.yaml`; canonical YAML wins.
...
- Runtime close, sidebar cleanup, hiring, scoring: expand `docs/agents/workflows.yaml`; sidebar cleanup follows `runtime.sidebar_cleanup_success_path` and is destructive/no-backup only after exact authorization.
