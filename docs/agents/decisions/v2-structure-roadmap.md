...
- `docs/templates/agents/` as target-specific deployment starters only.
- `scripts/`, `schemas/`, `tests/`, `artifacts/`, and
  `.github/workflows/` as explicit v2 execution boundaries.
## Current Rule
Future changes must preserve canonical-source uniqueness, starter cleanliness,
ignored runtime boundaries, deployed file set ownership, and size gates.
